...
Опционально — заполнить сайт демо-данными (тренеры, расписание, тарифы, статьи
блога с картинками из `media/`):

```bash
python manage.py seed_demo
```

Команда идемпотентна (повторный запуск не плодит дубли). Флаг `--flush` очистит
старые демо-данные перед заполнением. Если выполнили `seed_demo`, отдельный
`createsuperuser` не нужен — она создаёт админа `admin / admin12345`
(**обязательно смените пароль** в админке после первого входа).
